...
def process_resume_job(resume_file, job_desc_text):
    """Process resume and JD, generate skills comparison and ATS suggestions"""
    # Process resume
    ...
    # Generate skills comparison and ATS suggestions
...
def update_resume_job(resume_file, job_desc_text, app_state, metadata_state, jd_state):
    """Update resume or JD and reprocess"""
...
with gr.Blocks(theme=custom_theme, css=css, title="Adaptive RAG Resume Agent") as demo:
    ...
    <div style="text-align:center; padding:2rem;">
        ...
        <p style='color:#555; margin-top:0.5rem;'>
            Upload your resume and optionally a job description. Press Process to view skills and chat insights.
        </p>
    </div>
    """)
    ...
    # --- Update Action ---
    def update_action(new_resume, new_jd, app_s, meta_s, jd_s):
        return update_resume_job(new_resume, new_jd, app_s, meta_s, jd_s)
    ...
    # --- Chatbot ---
    def submit(message, history, app_s, meta_s, jd_s):
        history = chat_fn(message, history, app_s, meta_s, jd_s)
