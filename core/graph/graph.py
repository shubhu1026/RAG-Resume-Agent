...
# from core.routing import build_routing_chain, should_use_vectorstore
...
#     def retrieve(state):
#         """
#         Expand question, retrieve using the vectorstore, de-duplicate, and store 'documents' as plain docs.
#         """
#         question = state["question"]

#         # small safety: if vector_db exposes as_retriever, use it; else fallback to similarity_search
#         try:
#             base_retriever = vector_db.as_retriever(search_kwargs={"k": 6})
#         except Exception:
#             base_retriever = None

#         # Expand query into multiple perspectives (keeps your existing behavior)
#         expanded_queries = expand_query(question, num_variations=3)
#         retrieved_docs = []

#         # Use a retriever when available for efficiency
#         if base_retriever:
...
#             key = getattr(d, "page_content", str(d))[:800]  # short key
#             if key not in unique:
#                 unique[key] = d

#         docs = list(unique.values())

#         # attach docs to state as Document objects (so grade_generation can inspect)
...
#         query = state["question"]
#         jd = state.get("job_description", "")
#         if hasattr(jd, "content"):
#             jd = jd.content
#         jd = jd.strip() if jd else ""

#         # Use a retriever if available
#         try:
#             base_retriever = vector_db.as_retriever(search_kwargs={"k": 6})
#         except Exception:
#             base_retriever = None

#         # Attempt to use ContextualCompressionRetriever (saves tokens)
#         compressed_retriever = None
#         try:
...
#             )
#             compressor_chain = LLMChain(llm=ChatOpenAI(model_name=MODEL_NAME, temperature=0), prompt=compressor_prompt)
#             extractor = LLMChainExtractor(llm_chain=compressor_chain)
#             if base_retriever:
#                 compressed_retriever = ContextualCompressionRetriever(base_retriever=base_retriever, compressor=extractor)
#         except Exception as e:
#             # not all environments have these classes; fallback to base_retriever
#             compressed_retriever = base_retriever

#         # Get context docs
#         if compressed_retriever:
#             context_docs = compressed_retriever.get_relevant_documents(query)
#         elif base_retriever:
#             context_docs = base_retriever.get_relevant_documents(query)
#         else:
#             context_docs = vector_db.similarity_search(query, k=3)

#         # Build context text (concise)
#         context_text = "\n\n".join([d.page_content for d in context_docs])

#         # Prefer job description if present
#         if jd:
#             llm_input = f"Job Description:\n{jd}\n\nResume Info:\n{context_text}"
#         else:
#             llm_input = f"Resume Info:\n{context_text}"

#         # Run RAG chain (your existing LLM chain)
#         response = rag_chain.run(context=llm_input, question=query)
...
#         # --- STEP 1: Quick keyword-based rule (your old one)
#         if jd_summary and any(word in q_lower for word in ["company", "role", "position", "salary", "location"]):
#             return "retrieve"

#         # --- STEP 2: NEW — Embedding similarity check (fast heuristic)
#         try:
#             # vector_db is available in outer scope (closure from build_workflow)
#             if should_use_vectorstore(question, vector_db, threshold=0.70, k=3):
#                 print("[Routing] → Heuristic triggered: VECTORSTORE")
#                 return "retrieve"
#         except Exception as e:
#             print("[Routing] Heuristic failed:", e)

#         # --- STEP 3: Fallback — LLM-based router
#         route = routing_chain.run({
...
#         print(f"[Routing] LLM route decided: {route}")
